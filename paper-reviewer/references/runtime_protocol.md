...
- 对每个检查对象运行 `python scripts/reference_guide.py nav --domain <canonical-domain-name> --review-stage macro --paper-section <section>`。
- 只在需要具体标准时运行 `python scripts/reference_guide.py show --domain <canonical-domain-name> --heading "<exact-heading>"`。
- 将每个宏观问题写成独立 `finding`。
...
- 对每个章节 pass 运行 `python scripts/reference_guide.py nav --domain <canonical-domain-name> --review-stage micro --paper-section <section>`。
- 审阅 `introduction` 或 `literature` 时，通过 `nav` 中的 cross-cutting review topics，或 `python scripts/reference_guide.py topic show --topic scientific-introduction-related-work --heading "<exact-heading>"`，检查问题定位、相关工作组织、研究缺口、引用支撑和作者判断。
...
- 对领域特定报告规范使用 `python scripts/reference_guide.py nav --domain <canonical-domain-name> --review-stage formal --paper-section <section>`。
- 审阅 `references` 时，通过 `python scripts/reference_guide.py nav --domain <canonical-domain-name> --review-stage formal --paper-section references` 中的 cross-cutting review topics，或 `python scripts/reference_guide.py topic show --topic bibliography-format-diagnostics --heading "<exact-heading>"`，识别可见参考文献条目的格式、著录、切分、一致性和抽取噪声问题。
- 通过 `python scripts/reference_guide.py nav --domain <canonical-domain-name> --review-stage formal --paper-section <section>` 中的 cross-cutting review topics，或 `python scripts/reference_guide.py topic show --topic ai-style-diagnostics --heading "<exact-heading>"`，检查模板化表达、低信息密度、过度圆滑、作者判断不足和 AI 使用合规线索。
...
```bash
python scripts/review_runtime.py export --run-root <RUN_ROOT>
```
